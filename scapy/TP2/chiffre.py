...
from Cryptodome.Cipher import AES  # Modifiez cette ligne
...
def send_encrypted_broadcast_message(message, iface, broadcast_ip='192.168.1.255'):
    ...
    packet = IP(dst=broadcast_ip) / UDP(dport=12345) / Raw(load=encrypted_message)

    # Envoyer le paquet en broadcast
    send(packet, iface=iface)

# Définir le message à envoyer
message_to_send = "tu te tais ptn"
